...
#
loop:
  lw $t4, pixels($t0)	# $t4 assigned to pixels[i]
  beq $t4, -1, exit 	# if -1, then break
...
#   jal rgb_to_gray
#
  andi $t1, $t4, 0xff	# $t1 is bleu
  srl $t4, $t4, 8	# $t4 >> 8
  
  andi $t2, $t4, 0xff 	# $t2 is green
  srl $t4, $t4, 8	# $t4 >> 8
  
  andi $t3, $t4, 0xff # $t3 is red
  
  jal rgb_to_gray
# This procedure call computes and returns the gray value.
#
...
#
  ...
  add $a0, $0, $t6	# Gray into $a0
  syscall
  
  addi $v0, $0, 4	# System call 4
  la $a0, newline	# Newline into $a0
  syscall
...
rgb_to_gray:            # procedure to calculate gray = (red + green + blue) / 3
  ...
  add $t5, $t3, $t2	# Red plus green into temp
  add $t5, $t5, $t1	# Temp plus bleu
  div $t6 $t5, 3	# Temp divided 3
